...
struct AreaLight {
  ...
  std::unique_ptr<BSDF> create_bsdf(const bounding_sphere_t&, uint32_t light_id,
                                    bool diffuse) const;
...
class AreaLights : public Geometry {
 public:
  ...
  const size_t addLight(const string& name, uint32_t material_index,
                        const vec3& position, const vec3& direction,
                        const vec3& up, const vec3& exitance, const vec2& size,
                        bool diffuse);
