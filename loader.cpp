...
Stream& operator<<(Stream& stream, const aiString& string) {
  return stream << string.C_Str();
}

template <class Stream>
Stream& operator<<(Stream& stream, const aiColor3D& color) {
  return stream << "aiColor3D(" << color.r << ", " << color.g << ", " << color.b
                << ")";
}

template <class Stream>
Stream& operator<<(Stream& stream, const aiVector2D& vector) {
  return stream << "aiVector2D(" << vector.x << ", " << vector.y << ")";
}

template <class Stream>
Stream& operator<<(Stream& stream, const aiMatrix4x4& matrix) {
  return stream << "aiMatrix4x4("
...
template <class Stream>
Stream& operator<<(Stream& stream, const aiVector3D& vector) {
  return stream << "aiVector3D(" << vector.x << ", " << vector.y << ", "
                << vector.z << ")";
}

template <class Stream>
Stream& operator<<(Stream& stream, aiLightSourceType type) {
  switch (type) {
    case aiLightSource_UNDEFINED:
      return stream << "aiLightSource_UNDEFINED";
    case aiLightSource_DIRECTIONAL:
      return stream << "aiLightSource_DIRECTIONAL";
    case aiLightSource_POINT:
      return stream << "aiLightSource_POINT";
    case aiLightSource_SPOT:
      return stream << "aiLightSource_SPOT";
    case aiLightSource_AREA:
      return stream << "aiLightSource_AREA";
    default:
...
template <class Stream>
Stream& operator<<(Stream& stream, const aiCamera& camera) {
  return stream << "aiCamera { "
...
template <class Stream>
Stream& operator<<(Stream& stream, const aiLight& light) {
  return stream << "aiLight { "
...
template <class Stream>
Stream& operator<<(Stream& stream, const aiNode& node) {
  return stream << "aiNode { mChildren, mMeshes, "
                << "mName = " << node.mName << ", "
...
vec3 toVec3(const aiColor3D& v) { return vec3(v.r, v.g, v.b); }
...
template <class T>
T property(const aiMaterial* material, const char* key) {
  return T();
}

template <>
bool property(const aiMaterial* material, const char* key) {
  int result = 0;
...
template <>
float property(const aiMaterial* material, const char* key) {
  float result = 0;
  material->Get(key, 0, 0, result);
  return result;
}
...
template <class Stream>
Stream& operator<<(Stream& stream, const aiMaterial& material) {
  stream << "aiMaterial { ";

  for (unsigned i = 0; i < material.mNumProperties; ++i) {
    if (material.mProperties[i]->mType == aiPTI_String) {
      aiString result;
      aiGetMaterialString(&material, material.mProperties[i]->mKey.C_Str(), 0,
                          0, &result);
      ...
      aiGetMaterialFloatArray(&material, material.mProperties[i]->mKey.C_Str(),
                              0, 0, result, &pMax);
...
Mesh aiMeshToMesh(const aiMesh* mesh, uint32_t materials_base,
                  entity_type type) {
  ...
  if (mesh->mBitangents == nullptr || mesh->mTangents == nullptr) {
  ...
  result.material_id =
      encode_material(mesh->mMaterialIndex + materials_base, type);

  return result;
}

void load_meshes(const aiScene* scene, Materials& materials,
                 vector<Mesh>& meshes) {
  ...
  for (size_t i = 0; i < scene->mNumMaterials; ++i) {
    ...
    if (property<bool>(material, "$mat.blend.transparency.use")) {
      ...
      materials.bsdfs.push_back(
          unique<BSDF>(new DiffuseBSDF(diffuse(material))));
    } else {
      vec3 total = specular(material) + diffuse(material);

      if (total.x > 1.0f || total.y > 1.0f || total.z > 1.0f) {
        std::cerr << "Invalid material: " << name(scene->mMaterials[i]) << " "
                  << specular(material) + diffuse(material) << std::endl;
      }

      materials.bsdfs.push_back(unique<BSDF>(new PhongBSDF(
          diffuse(material), specular(material), shininess(material))));
    }
  }

  for (size_t i = 0; i < scene->mNumMeshes; ++i) {
    meshes.push_back(
        aiMeshToMesh(scene->mMeshes[i], materials_base, entity_type::mesh));
  }
}
...
bounding_sphere_t compute_bounding_sphere(const std::vector<Mesh>& meshes) {
  ...
  for (auto&& mesh : meshes) {
    for (auto&& vertex : mesh.vertices) {
      result.radius =
          glm::max(result.radius, glm::distance2(result.center, vertex));
    }
  }
...
void load_lights(const aiScene* scene, Materials& materials, AreaLights& lights,
                 vector<Mesh>& meshes) {
  ...
  for (size_t i = 0; i < scene->mNumLights; ++i) {
    if (scene->mLights[i]->mType == aiLightSource_AREA) {
      ...
      lights.addLight(
          toString(light->mName), materials.names.size(),
          toVec3(light->mPosition), normalize(toVec3(light->mDirection)),
          normalize(toVec3(light->mUp)), toVec3(light->mColorDiffuse),
          toVec2(light->mSize), light->mDiffuse);

      meshes.push_back(lights.light(i).create_mesh(materials.names.size(),
                                                   toString(light->mName)));

      materials.names.push_back(toString(light->mName));
      materials.bsdfs.push_back(
          lights.light(i).create_bsdf(bounding_sphere, i, light->mDiffuse));
    }
  }
}
...
shared<Scene> loadScene(string path) {
  ...
  shared<Scene> result = make_shared<Scene>(move(cameras), move(materials),
                                            move(meshes), move(lights));
